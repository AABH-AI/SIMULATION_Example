...
| File | Role |
|---|---|
| `BPA_FORCASTING_MOCK.HTML` | **Active development file, live as the "TET BPA — Forecasting Suite (Active)" card** — rebuilt version of the dashboard (sessions 14+); 5 home tiles: Forecast Accuracy, Actuals Profiling, Data Management, a non-clickable **What If** placeholder (Work in Progress), and **BTC Adjustment Simulator** (links out to `template_ui/btc_adjustment_simulator_v2.html` — see Module Structure below). **On branch `master_html` only** (local, not merged into `master`): the filter panel (`#right-panel`) was moved from the right-side rail to a collapsible top bar — same markup/JS, layout-only CSS change. See Session 55. |
