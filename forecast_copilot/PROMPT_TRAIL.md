...
> Last updated: 2026-07-24 (ASU/SR/Dispatch toggle + Final Forecast year view, branch `review-hn-new`)
...
## Review pass — ASU/SR/Dispatch chart toggle (ASU Simulation) + next-year view (Final Forecast)
**Date**: 2026-07-24 | **Branch**: `review-hn-new` (new branch off `hn-new`, pushed for review — `hn-new` itself untouched)
**Files**: `forecast_copilot/ASU Simulation — Forecast Copilot.html`, `forecast_copilot/Final Forecast — Forecast Copilot.html`, this file, `HANDOFF.md`, `README.md`
**Prompt**: review `hn-new` for guideline compliance and mergeability, then (1) ASU Simulation: add ASU/SR/Dispatch buttons that show one forecast graph at a time, (2) Final Forecast: default to next fiscal year with an option to compare against the previous year. Told explicitly to keep both changes static (seeded/dummy data only) — don't wire in the real Excel/`serve.py` path for this pass.

**Guideline audit** (checked against `CLAUDE.md` + this folder's own `README.md`/`HANDOFF.md`/`BUILD_PLAN.md`, which the folder's own docs say win over the canonical root docs for this folder): no real violations. Highcharts (vs. the root suite's Chart.js) is a documented, intentional deviation — this folder is its own separate product. One flag surfaced, not acted on this pass: Highcharts 11.4.8 here is loaded under its **free non-commercial/research license** per this README's own licensing note; this is a public repo, so if the suite is used for real Dell business planning that license may not apply — left for a later decision. `serve.py` already degrades gracefully to seeded data (visible "Simulated data" badge) with no server present, so nothing here depends on it.

**ASU Simulation — one chart, ASU/SR/Dispatch toggle**: replaced the always-visible 3-chart `.grid-3` (separate ASU/SR/Dispatch panels) with a single panel: a 3-button toggle group (`#asu-metric-toggle`) above one chart (`#chart-metric`). `ASU_METRIC_CONF` maps each metric to its title + `fc_engine.js` series keys (`asuBase/asuAdj`, `srBase/srAdj`, `dspBase/dspAdj`); clicking a button sets `_asuMetric` and re-renders — only the selected metric's Base-vs-Scenario graph shows, matching the metric-switcher pattern already used elsewhere in the suite (e.g. Forecast Trend's `switchChartMetric`). No `fc_engine.js` changes.

**Final Forecast — next-year default + previous-year compare**: the page was scoped to a single selected quarter (13 weeks); reworked to a full fiscal year. `fcSelectedYear()` reads the year off the existing Fiscal Quarter filter; `fcNextYear()` is that +1. `fcComputeQuarter(quarterStr)` and `fcComputeYear(year)` are new page-local helpers — `fcComputeYear` calls `fcComputeQuarter` for each of a year's 4 quarters (via the engine's own existing `fcSnapshotPlan`/`fcComputeFor`, the same non-destructive apply-then-restore mechanism the Compare-scenarios modal already uses) and concatenates the 4 quarters' weekly arrays into one 52-week view; status checks (`meetsAOP` etc.) aggregate across the 4 quarters. `fc_engine.js` itself is unchanged, so the other 5 pages are unaffected. A "Compare with previous year" checkbox (`#ff-compare-toggle`) adds a 6th "Prev. Year Final" line to the chart, computed the same way for `fcSelectedYear()`. `fcPlanFingerprint()` and `fcBuildPublishPayload()` were updated to key off the resolved year + compare flag instead of the raw quarter string, so Submit's disable-until-changed behavior and the published payload's slice both reflect "Fiscal Year" rather than "Fiscal Quarter".

**Verification**: both files' inline scripts parsed clean with `node -e "new Function(code)"` (extracted via regex, no `src=` scripts). Confirmed no other file in `forecast_copilot/` references the removed `chart-asu`/`chart-sr`/`chart-dsp` ids or relies on `.grid-3` (Dashboard has its own distinct `chart-asu-trend` etc. ids, unaffected). Not verified in an actual browser this pass (no browser available in this session) — recommend a manual load-and-click pass on both pages before merging `review-hn-new`.

**Not done / left as follow-ups**: root `IMP_DOCS/` intentionally left untouched (per this folder's own stated convention that it documents the pre-refactor state); the Highcharts licensing flag above; `review-hn-new` has not been merged into `hn-new` or `master` — it's a separate branch pushed for the branch owner to review.
