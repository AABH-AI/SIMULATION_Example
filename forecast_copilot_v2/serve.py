...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _send_json(self, obj, status=200):
        ...
        self._cc_done = True   # suppress the static no-cache header added in end_headers()
        self.end_headers()
        if self.command != "HEAD":
            self.wfile.write(body)

    def end_headers(self):
        # Force browsers to revalidate static assets (HTML/JS/CSS) on every load so
        # edits show on a normal refresh — no hard-reload needed. SimpleHTTPRequestHandler
        # sends Last-Modified, so unchanged files still return an efficient 304.
        if not getattr(self, "_cc_done", False):
