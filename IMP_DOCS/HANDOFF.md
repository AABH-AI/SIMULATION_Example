...
> Last updated: 2026-08-31 | Owner: Arnav Bhargava (arnav.bhargava@alignedautomation.com)
...
| `btc_react_simulator/app/**` | **Separate product** — React port of `template_ui/btc_adjustment_simulator_v2.html` (Vite + React 19 + Zustand + Highcharts). Faithful behavior port, P5 parity-verified vs original (2026-08-31). See `btc_react_simulator/imp_docs/REACT_MIGRATION_PLAN.md` §9. Local only — repo not under git yet. |
...
- `btc_react_simulator/app/**` — **(2026-08-31)** React port of `template_ui/btc_adjustment_simulator_v2.html`, complete. Phases P0–P5 done; `npm run build` + `npm run lint` clean. P5 side-by-side parity vs original: PASS (data byte-identical, ASU/SR/Disp full-table checksums identical, driver + publish-edit compute identical, all interactions verified, 0 console errors). Local only — repo not yet under git (connect to `AABH-AI/SIMULATION_Example` on branch `hn-new` before committing). Full record in `btc_react_simulator/imp_docs/REACT_MIGRATION_PLAN.md` §9.
