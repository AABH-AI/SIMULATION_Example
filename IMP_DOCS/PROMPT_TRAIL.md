...
## Session 27 — BTC Adjustment Simulator: React port + P5 parity verification
**Files**: `btc_react_simulator/app/**` (new React app), `btc_react_simulator/imp_docs/REACT_MIGRATION_PLAN.md`, `IMP_DOCS/HANDOFF.md`, `IMP_DOCS/PROMPT_TRAIL.md`
**Prompts**: "verify p5" → "check SR/Disp/Publish numeric parity" → "verify p5 leftovers, polish, update imp_docs"

**What was done**:
- Verified the React port of `template_ui/btc_adjustment_simulator_v2.html` (Vite + React 19 + Zustand + Highcharts, in `btc_react_simulator/app/`) against the original **side-by-side** in the browser (React `:5173`, original served on `:8899`). Result: **full parity, P5 PASS**.
- Data byte-identical (`src/data/btc_data.json` = `template_ui/input/btc_data.json`). ASU/SR/Disp full-table checksums identical across all 104 weeks; driver compute identical (NC% 120 → Adjusted ASU 6,970,017); Publish edited-row identical (`NC_Adj=60000` → `…|5,359,531|…`).
- Verified interaction paths: export + Pass# bump (Blob fallback — FS Access dir-write is Chromium/secure-ctx only), decline CSV import, allocation modal, comment popover (body portal). Zero console errors across the sweep.
- Polish: oxlint 4 warnings → 0 (regex escapes in `btcEngine.js`, unused `state` import in `RateView.jsx`, unused `get` param in `useBtc.js`); `vite build` clean; rewrote `app/README.md` (was Vite boilerplate).
- Recorded full results in `REACT_MIGRATION_PLAN.md` §9.

**Gotcha**: React commits table edits on `focusout`, not raw `blur` — a synthetic `new Event('blur')` won't commit and looks like a stale-value bug. Also reset `localStorage` between app comparisons.

**Known-open**: repo not under git yet (connect to `AABH-AI/SIMULATION_Example`, branch `hn-new`); AOP slider bound is an intentional redesign (`aopSliderMax` = 1.5×peak), not a defect; real `outputs/` file-write needs one manual gesture to confirm.
